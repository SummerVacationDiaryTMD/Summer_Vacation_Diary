...
4. 핵심 단어·문장 첨삭 표시, 감성 태그, 선생님의 한마디가 포함된 그림일기 카드를 미리 본다.
...
| AI 분석 | OpenAI Chat Completions API 또는 로컬 mock | 사진·일기 키워드, 감정, 첨삭 대상, 한마디 생성 |
...
실제 AI 응답은 JSON으로 받아 키워드 배열, 감정 배열, 첨삭 단어, 첨삭 문장, 한마디를 검사한다. 본문에 실제로 존재하지 않는 첨삭 단어나 문장은 버려 잘못된 위치가 표시되지 않게 한다.

### 4.5 미리보기 표시

`PreviewStep`은 날짜·날씨, 사진, 제목, 본문, 한마디 순서로 종이 일기장 형태의 카드를 그린다. 분석된 감정과 사진·일기 키워드는 중복을 제거한 뒤 최대 6개 태그로 표시한다.
...
| 파일 | 기능 |
| --- | --- |
| `.granite/app.json` | Granite용 앱 식별자와 권한 목록의 간단한 메타데이터다. 현재 `appName`은 `my-summer-story`, 권한은 빈 배열이다. |
...
| `WriteStep.tsx` | 제목, 날짜, 날씨, 일기 본문 입력 화면이다. 제목 30자, 본문 20~55자 규칙을 안내하고 공백만 입력한 값을 오류로 표시한다. 모든 변경을 상위 `DiaryDraft`에 즉시 반영한다. |
| `PreviewStep.tsx` | 완성 전 일기 카드를 렌더링한다. 분석 상태에 따라 로더, 오류·재시도, 한마디·태그를 표시하며 본문 첨삭 표시(동그라미/밑줄)를 프레임에 맞춘 원고지 그리드 + 오버레이로 렌더링한다. |
...
| 본문 | 최대 55자(11×5 원고지), 앞뒤 공백을 제외한 길이가 최소 20자여야 한다. |
...
| AI 응답 | 배열 길이와 문자열 여부를 검사하고, 한마디가 없으면 실패로 처리한다. 첨삭 대상은 실제 본문 부분 문자열만 허용한다. |
