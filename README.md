...
| 일기 작성        | 제목, 날짜, 날씨, 13×5 원고지 분량의 본문 입력                                  | 구현 완료 |
...
사진 변환과 일기 검사는 `검사 받기`를 눌렀을 때 필요한 작업만 실행합니다. 두 작업은 사용자에게 하나의 AI 검사 기회로 제공되며 총 3회 사용할 수 있습니다. 같은 사진의 그림과 같은 입력의 분석 결과는 재사용합니다.
...
| 로컬 저장     | Web Storage, Web Crypto                       | 작업 사본, 변환 캐시, 익명 식별값 |

라우터, 전역 상태 라이브러리, 사용자 계정 인증, 일기 보관용 데이터베이스, 자동 테스트 프레임워크는 사용하지 않습니다. 서버 사용량 제한은 Supabase의 `diary_ai_rate_limits` 테이블에 집계합니다.
...
├── src/
│   ├── components/         # 업로드, 작성, 미리보기, 자르기, 완료 모달
...
- 완성 이미지는 기기에 저장합니다. 일기를 기기에 보관하는 서비스 모듈(`src/services/diaryStore.ts`)은 있지만 아직 어떤 화면과도 연결하지 않아, 앱에서 지난 일기 목록을 볼 수는 없습니다.
