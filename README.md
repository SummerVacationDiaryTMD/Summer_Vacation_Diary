...
| 일기 달력        | 완성 일기를 날짜별로 자동 보관하고 같은 초안은 한 날짜에만 유지                 | 구현 완료 |
...
사진 변환과 일기 검사는 `검사 받기`를 눌렀을 때 필요한 작업만 실행합니다. 두 작업은 사용자에게 하나의 AI 검사 기회로 제공되며 총 3회 사용할 수 있습니다. 같은 사진의 그림과 같은 입력의 분석 결과는 재사용합니다. 사용량이 이미 소진되어 실행할 작업이 없으면 처리 애니메이션을 건너뛰고 원본 기반 미리보기를 바로 표시합니다. 완성한 JPEG를 일기 달력에 저장하면 `일기가 달력에 저장되었어요!` 토스트로 결과를 알립니다.
...
- 완성한 일기는 일기 달력에 자동 보관되며 날짜별 최대 3개까지 저장됩니다. 같은 초안을 날짜만 바꿔 다시 완성하면 이전 기록을 새 날짜로 이동해 하나만 유지합니다. 토스 앱에서는 `Storage`, 일반 브라우저에서는 `localStorage`를 사용합니다.
