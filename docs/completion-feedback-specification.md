...
| 달력 보관     | `saveDiary()`가 기기 저장소에 record와 index 저장 | `일기가 달력에 저장되었어요!` 토스트 표시 |
...
- 미리보기에는 `일기 완성하기`를 눌러야 달력에 저장된다는 안내를 계속 표시합니다.
...
- [ ] 자동 보관 성공 직후 `일기가 달력에 저장되었어요!` 토스트가 한 번 표시된다.
- [ ] 같은 초안의 날짜를 바꿔 재저장하면 새 날짜에만 남고 같은 날짜는 최대 3개로 제한된다.
