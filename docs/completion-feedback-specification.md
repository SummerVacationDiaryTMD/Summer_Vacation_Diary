...
| 달력 보관     | `saveDiary()`가 기기 저장소에 record와 index 저장 | `일기 달력에 저장했어요.` 토스트 표시 |
...
- 날짜별 최대 3개
- 같은 완성 이미지를 다시 저장하면 중복 record를 만들지 않음
- index에는 이미지 없는 요약만 저장하고, JPEG는 일기별 key에 저장
- 보관 성공 시 `일기 달력에 저장했어요.` 토스트 표시
...
- [ ] 자동 보관 성공 직후 `일기 달력에 저장했어요.` 토스트가 한 번 표시된다.
- [ ] 같은 이미지가 중복 보관되지 않고 같은 날짜는 최대 3개로 제한된다.
