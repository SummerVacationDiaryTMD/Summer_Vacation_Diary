...
| 달력 보관     | `saveDiary()`가 기기 저장소에 record와 index 저장 | `일기가 달력에 저장되었어요! (현재 개수/3)` 토스트 표시 |
...
- 보관 성공 시 `일기가 달력에 저장되었어요! (현재 개수/3)` 토스트 표시
...
완성 모달은 달력 상세 뷰어의 `저장 및 공유`를 누르면 열리며 다음 요소를 제공합니다.
...
달력 상세 뷰어의 `저장 및 공유`는 완성 모달을 엽니다. 모달에서 보관한 JPEG를 기기에 저장하거나 앱 진입 링크를 공유할 수 있습니다.
...
- [ ] 상세 뷰어의 `저장 및 공유`를 눌렀을 때만 완성 모달이 열린다.
- [ ] 완성 모달에는 이미지 미리보기가 없고 저장·공유 설명과 버튼만 표시된다.
- [ ] 완성 모달에서 하늘색 앱 공유는 왼쪽, 빨간 이미지 저장은 오른쪽에 반씩 표시되고, 그 아래 창 닫기가 아이콘 없는 크림색 공통 버튼 스타일로 표시된다.
- [ ] 완성 모달의 저장·앱 공유·창 닫기가 동작한다.
- [ ] 자동 보관 실패 시 달력이나 완성 모달로 이동하지 않고 미리보기를 유지한다.
- [ ] 자동 보관 성공 직후 `일기가 달력에 저장되었어요! (현재 개수/3)` 토스트가 한 번 표시된다.
- [ ] 자동 보관 성공 직후 해당 날짜에 도장이 찍히고, 1.5초 뒤 방금 저장한 기록이 자동으로 열린다.
- [ ] 완성 후 달력의 `돌아가기`는 그림 선택 화면으로, 날짜 관리 진입 후에는 일기 쓰기 화면으로 이동한다.
- [ ] 같은 초안의 사진·본문은 유지하고 날짜만 바꾸면 새 날짜에만 남고 같은 날짜는 최대 3개로 제한된다.
