...
| 여섯 번째 일기가 보관되지 않음    | 같은 날짜에는 서로 다른 완성 일기를 최대 5개까지 보관               |
...
4. `일기 완성하기` 후 JPEG 자동 보관과 `일기가 달력에 저장되었어요! (현재 개수/5)` 토스트 확인
5. 완성 모달에서 다운로드와 앱 링크 공유 확인
6. 업로드 화면의 `나의 기록 보기`에서 해당 날짜 도장 확인
7. 같은 사진·본문의 날짜 변경 시 이전 기록 교체, 사진 또는 본문 수정 후 다른 날짜 저장 시 별도 기록 유지, 이미지 내보내기와 삭제 확인
