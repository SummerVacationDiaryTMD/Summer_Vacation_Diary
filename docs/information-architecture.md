...
앱에는 URL 라우터와 전역 메뉴가 없습니다. `src/App.tsx`의 React 상태가 온보딩, `upload → write → preview` 제작 단계와 독립 목적지인 `calendar`를 전환합니다. 자르기·동의·완성·보관 일기 상세는 현재 화면 위에 오버레이로 열립니다.
...
- 자르기와 일기 뷰어는 `role="dialog"`와 `aria-modal`, 동의·완성·삭제 확인 화면은 TDS `Modal`을 사용합니다.
