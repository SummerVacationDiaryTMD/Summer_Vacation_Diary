...
| ------------- | ----------------- | ----------------------------------------- | --------------------------- |
| `analysis.ts` | `ANALYSIS_PROMPT` | `analyze` action (diary + photo analysis) | chat `system` message       |
| `sketch.ts`   | `SKETCH_PROMPT`   | `sketch` action (photo → colored pencil)  | `images/edits` prompt field |
...
- **Dashboard** (the team's usual path): in the Functions editor, create or
  ...
  `prompts/analysis.ts`, etc.). Keep "Enforce JWT Verification" OFF —
