...
Hard challenge constraint: the word "AI" must never appear in the app name. `appName` stays `my-summer-story`; the user-facing display name ("나의 여름방학 일기") lives in both the console and `granite.config.ts` `brand.displayName` (the 2.x schema requires it), while the icon is managed by console upload — `brand.icon` stays an empty placeholder.
...
- `src/services/` is the external-AI boundary, and every service is mock-first: without the two `VITE_SUPABASE_*` public settings the app runs deterministic local mocks (체험 모드), so the full flow works offline. With Supabase configured, both operations call the `diary-ai` Edge Function; the OpenAI key and model settings never ship in the client.
  ...
  - `styleTransfer.ts` — requests photo → colored-pencil sketch conversion through the Edge Function. It owns the sketch ledger ticket, because it is the one function that is 1:1 with a paid request (test and mock mode short-circuit above it).
  - `sketchLedger.ts` — counts a drawing at dispatch instead of at response, keyed by the cropped photo data URL. One ticket per photo: the counter moves the moment the request goes out, and moves back only for failures the server does not charge for (`content-blocked` and `invalid-image` are charged; quota 429s and region 403s are not, since the server rejects them before reserving). This steers a normal user's flow — it is not enforcement, and a modified client ignores it.
...
- The versioned `supabase/functions/diary-ai/` source enforces per-device daily, per-IP short-window and daily, and service-wide daily quotas — all atomically in one RPC — through the migration in `supabase/migrations/`. Quotas are per action (`sketch`, `analyze`) except the IP scope, which sums both. Accounting is reserve-then-refund: the Function consumes before the paid call and refunds only failures that are not the user's fault (every HTTP 400 is the user's fault and is never refunded). Raw identifiers are salted and hashed before storage; limit values live together in the Function's `USAGE_LIMITS` constant, and every response carries the caller's current quota snapshot. Because the migration drops and recreates its functions, only the newest file in `supabase/migrations/` is meant to be run — the team pastes it into the Dashboard SQL Editor rather than using `supabase db push`, so older files there are history, not a chain to replay.
- Function logs go to the Supabase Dashboard (Logs → Edge Functions). Every request logs one line — action, country, elapsed ms — under a short per-request id, because concurrent sketches would otherwise interleave unreadably; failures add the upstream OpenAI status/code and whether the request was refunded. Verbose lines (input sizes, quota counters, non-JSON model output, stack traces) are gated behind the `DIARY_AI_DEBUG` Secret, since Supabase caps a function at 100 log events per 10 s and Free-plan logs are kept for only 1 day. Never log the diary text, the photo, or a raw IP: the first two are covered by the app's consent notice and the third is hashed before storage precisely so it does not sit in the clear.
- The Function also refuses non-Korean callers (`region-blocked`, HTTP 403), checked before the quota reservation so a refused call consumes nothing. The country comes only from request headers (`COUNTRY_HEADERS`); Supabase documents no such header for Edge Functions, so **an unknown country is allowed through** — failing closed would take the app down the moment the signal disappears, and the service-wide daily cap is what actually bounds spend. `quota-status` is never region-gated: it is how the client learns it is blocked, and every snapshot carries `region: { allowed, country }` so `country` doubles as the probe for whether the header exists at all.
- All LLM prompt texts live alongside `index.ts` as content-only TypeScript modules (`supabase/functions/diary-ai/prompt_analysis.ts`, `prompt_sketch.ts` — one exported template literal each, no logic; there is no `prompts/` subdirectory). `index.ts` is the only entrypoint (a debug-only variant was removed once `index.ts` grew its own request logging), and regular imports bundle on every deploy path — including the team's Dashboard multi-file editor flow (recreate both prompt files there next to `index.ts`, since deploying `index.ts` alone fails on the imports, and keep Enforce JWT Verification off because Dashboard deploys ignore `config.toml`). Editing rules are in `supabase/functions/diary-ai/README.md`.
...
- Sandbox testing loop (the reason the project is on SDK 2.x): run `npm run dev`, then open `intoss://my-summer-story` in the sandbox app (needs the console-registered app and a Toss business login). The iOS simulator reaches localhost directly; Android needs `adb reverse tcp:8081 tcp:8081` plus `adb reverse tcp:5173 tcp:5173`; an iOS real device needs the Mac's Wi-Fi IP entered in the sandbox app.
