...
- All LLM prompt texts live in `supabase/functions/diary-ai/prompts/` as content-only TypeScript modules (`analysis.ts`, `sketch.ts` — one exported template literal each, no logic). Both entrypoints (`index.ts` and the debug variant) import the same modules, so prompts cannot diverge, and regular imports bundle on every deploy path — including the team's Dashboard multi-file editor flow (recreate the `prompts/` files there alongside `index.ts`, and keep Enforce JWT Verification off since Dashboard deploys ignore `config.toml`). Editing rules are in `prompts/README.md`.
